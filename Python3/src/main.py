...
alt_hist = []
plot_array_max_length = 80  # how many data points to hold in our arrays and graph
        ...
        while True:
            if datetime.now() > last_update + timedelta(milliseconds=update_interval * 1000):
                ...
                if (len(x_axis_counters) > plot_array_max_length):
                    ...
                    roll_history.append(roll_PID.output)
                    roll_setpoint_history.append(roll_PID.SetPoint)
                    pitch_history.append(pitch_PID.output)
                    pitch_setpoint_history.append(pitch_PID.SetPoint)
                    alt.append(current_altitude)
                    alt_hist.append(altitude_PID.SetPoint)
                ...
                p3.plot(x_axis_counters, alt, pen=1, clear=True)
                p3.plot(x_axis_counters, alt_hist, pen=0)
